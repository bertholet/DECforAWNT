...
The wedge product has the following properties, as is easy to show and is done e.g. in \cite{globalAnalysis}
\emph{
\begin{enumerate}
...
\subsubsection{Using the Wedge Product}
We start wedging $1$-forms. For two arbitrary 1-forms $\omega^1$, $\nu^1$ we get
...
\end{pmatrix}\]
 ...
 The space of one forms $\Lambda^1(\mathbb R^l)$ has dimension $l$ and is spanned by the special set of basis forms
\[de_i(a) := det_i(a) = a^i\]
...
\note{image} The action of pulling back $\omega$ using $h$ is denoted by $h^* \omega$. This mapping preserves the integral (check it by using the definitions!)
...
As we have seen in section \note{...} we can describe a set of simplices in a discrete manifold as a vector $\sigma$ of dimension $\# k$-$simplices$ consisting of plus/minus ones and zeros. The discrete form $\textbf{w}^k$ is a vector of the same dimension and the discrete integral over $\sigma$ is the scalar product of those two vectors
